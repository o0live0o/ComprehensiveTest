...
namespace ZiGongZJ
{
    public class CHCNetSDK
    {
        ...
        **************************************************/
        ...
        static CHCNetSDK()
        {
            //string dllPath = Path.Combine(Environment.Is64BitProcess ? "x64" : "x86", "HCNetSDK.dll");
            //dllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, dllPath);
            //dllPath = @"D:\IYASAKA\综检\ZiGongZJ\ZiGongZJ\bin\Debug\x64\HCNetSDK.dll";
            //CHCNetSDK.LoadLibraryA(dllPath);

            //int j = 0;

            //dllPath = Path.Combine(Environment.Is64BitProcess ? "x64" : "x86", "RecordDLL.dll");
            //dllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, dllPath);
            //CHCNetSDK.LoadLibraryA(dllPath);

            //dllPath = Path.Combine(Environment.Is64BitProcess ? "x64" : "x86", "PdCssVodClient.dll");
            //dllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, dllPath);
            //CHCNetSDK.LoadLibraryA(dllPath);

            //dllPath = Path.Combine(Environment.Is64BitProcess ? "x64" : "x86", "GetStream.dll");
            //dllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, dllPath);
            //CHCNetSDK.LoadLibraryA(dllPath);

            //dllPath = Path.Combine(Environment.Is64BitProcess ? "x64" : "x86", "AnalyzeData.dll");
            //dllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, dllPath);
            //CHCNetSDK.LoadLibraryA(dllPath);
        }
        ...
        **********************************************************/
        ...
        public static extern bool VODStreamSearch(IntPtr pSearchParam, ref IntPtr pSecList);
         ...
         [DllImport("PdCssVodClientPdCssVodClientPdCssVodClient.dll")]
